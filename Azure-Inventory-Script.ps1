...
#          Date: 02-06-2022                                                                     
#          Author: Sreenadha Reddy
#          Edit the necessary varibales as your orgnazation , like SMTP creds, To_Email ID, From_Email ID's --etc
...
function Invoke-GetAzureInventoryFunction{
    ...
    $working_directory = "C:\AzureInventory\AzureInventory-$timestamp"

    New-Item -Name C:\AzureInventory -ItemType Directory -Force
...
$path_to_store_inventory_csv_files = "C:\AzureInventory\AzureInventory-$timestamp\" + $subscription_name
...
Compress-Archive -Path "C:\AzureInventory\AzureInventory-$timestamp" -DestinationPath "C:\AzureInventory\AzureInventory-$timestamp\AzureInventory-$timestamp.zip" -Force

#### update the valid Azure Storage Account & Blob Container details as mentioned below if you want to push the zip into storage account

#$uploadstorage=Get-AzStorageAccount -ResourceGroupName 'RG of Storage account' -Name 'Storage account name'

#$storcontext=$uploadstorage.Context
#Set-AzStorageBlobContent -Container 'blob container name' -File "C:\AzureInventory\AzureInventory-$timestamp\AzureInventory-$timestamp.zip" -Context $storcontext

# Replace the SMTP , From & To email adresses with your ID's

$Username = "automation@abcxyz.com"
$toaddress = "azure-admin-team@abcxyz.com"

Send-MailMessage -From $Username -To $toaddress -Subject "Inventory Report from Azure Runbook-$timestamp" -SmtpServer "SMTP FQDN Endpoint or IP " -Credential 'domain01\admin01' -UseSsl -Attachments "C:\AzureInventory\AzureInventory-$timestamp\AzureInventory-$timestamp.zip" -Body "Hello Team \n Please find the attached Azure inventory Report \n \n "
